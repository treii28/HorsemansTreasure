...
AudioOutputI2SAve::AudioOutputI2SAve(int port, int output_mode, int dma_buf_count, int use_apll)
{
  this->portNo = port;
  this->i2sOn = false;
...
AudioOutputI2SAve::~AudioOutputI2SAve()
{
#ifdef ESP32
  if (i2sOn) {
...
bool AudioOutputI2SAve::SetPinout(int bclk, int wclk, int dout)
{
#ifdef ESP32
...
bool AudioOutputI2SAve::SetRate(int hz)
{
...
bool AudioOutputI2SAve::SetBitsPerSample(int bits)
{
...
bool AudioOutputI2SAve::SetChannels(int channels)
{
...
bool AudioOutputI2SAve::SetOutputModeMono(bool mono)
{
  this->mono = mono;
  return true;
}

bool AudioOutputI2SAve::begin()
{
  return true;
}

float AudioOutputI2SAve::getEnvelope()
{
    // use running average to come up with a logarithmic curve value from 0 to 7
    float vlog = (log(((float)aAve / 32768) * 8) / log(2)) + 4; // useful log(2) values range from -4 to +3, so offset by +4
    if(vlog < 0) vlog = 0.0; // redundant - handles library/math errors on very small values
    // return as value from 0 to 1
    return vlog / 7;
}

bool AudioOutputI2SAve::ConsumeSample(int16_t sample[2])
{
  int16_t ms[2];
  ...
  aAve = ((aAve * sWgt) + ((abs(ms[LEFTCHANNEL]) + abs(ms[RIGHTCHANNEL])) / 2)) / (sWgt + 1);
...
bool AudioOutputI2SAve::stop()
{
    int16_t silence[2] = {0, 0};
    ConsumeSample(silence);
#ifdef ESP32
  i2s_zero_dma_buffer((i2s_port_t)portNo);
#endif
  return true;
